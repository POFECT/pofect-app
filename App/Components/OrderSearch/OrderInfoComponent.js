...
export default function OrderInfoComponent({ searchTerm }) {
    ...
    useEffect(() => {
        ...
        MainApi.getOrderListByOrdNo(searchTerm, (data) => {
            ...
            if (data.response) {
                ...
                const rowData = headers.map(({ key, displayName }) => [displayName, data.response[key]]);
                    ...
                    <View >
                        <Table borderStyle={{ borderWidth: 1, borderColor: '#e4e6ee' }}>
                            ...
                            <Rows
                                data={tableData}
                                widthArr={widthArr}
                                style={[styles.row, { backgroundColor: '#F8F8FA' }]}
